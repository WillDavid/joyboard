<template>
  ...
  >
    <div class="card-header">
      ...
      <span class="card-project-tag" v-if="showProject">{{ getProjectName(task.project_id) }}</span>
    ...
    <div class="card-meta-row">
      <span class="card-responsaveis" v-if="resp1Name || resp2Name">
        <span class="resp-tag" v-if="resp1Name">{{ resp1Name }} <span class="resp-sigla">({{ resp1Sigla }})</span></span>
        <span class="resp-tag" v-if="resp2Name">{{ resp2Name }} <span class="resp-sigla">({{ resp2Sigla }})</span></span>
      </span>
...
function getUserSigla(id: string | null): string {
  if (!id) return ''
  const u = authStore.users.find(u => u.id === id)
  return u ? u.sigla : ''
}
...
const resp1Sigla = computed(() => getUserSigla(props.task.responsavel_1_id))
const resp2Sigla = computed(() => getUserSigla(props.task.responsavel_2_id))
